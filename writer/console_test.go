...
func TestFormatFields(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: fmt.Sprintf("🔖 %s=john • %s=123", color.ModernSlate.Render(writer.GetColorMode())+"user"+color.Reset, color.ModernSlate.Render(writer.GetColorMode())+"id"+color.Reset),
		},
	}
